...
For CESM/NorESM the run sequence is a free-format block stored in the
``nuopc.runconfig`` file, which the NUOPC driver ingests at start-up (via
``NUOPC_DriverIngestRunSequence``). It is generated by the CIME case-control
system from the coupling intervals you configure; the generator lives in
...
The run sequence below couples atmosphere, land, sea ice, ocean, river and
...
.. note::
   ...
   sequence in your run's ``nuopc.runconfig`` reflects the components and
